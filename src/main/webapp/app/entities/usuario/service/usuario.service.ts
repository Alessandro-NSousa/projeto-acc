...
export class UsuarioService {
  ...
  protected convertDateFromClient(usuario: IUsuario): IUsuario {
    return Object.assign({}, usuario, {
      dataCadastro: usuario.dataCadastro?.isValid() ? usuario.dataCadastro.toJSON() : undefined,
      ultimoAcesso: usuario.ultimoAcesso?.isValid() ? usuario.ultimoAcesso.toJSON() : undefined,
    });
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      ...
      res.body.ultimoAcesso = res.body.ultimoAcesso ? dayjs(res.body.ultimoAcesso) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((usuario: IUsuario) => {
        usuario.dataCadastro = usuario.dataCadastro ? dayjs(usuario.dataCadastro) : undefined;
        usuario.ultimoAcesso = usuario.ultimoAcesso ? dayjs(usuario.ultimoAcesso) : undefined;
      });
    }
    return res;
  }
